...
- Familiarity with Ansible or other configuration management tools`;
    ...
    // REMIND ME LATER
    // ============================================================
    const remindLaterToggle = document.getElementById('remindLaterToggle');
    const remindLaterForm = document.getElementById('remindLaterForm');
    const remindLaterSuccess = document.getElementById('remindLaterSuccess');
    const remindSubmitBtn = document.getElementById('remindSubmitBtn');
    const remindEmail = document.getElementById('remindEmail');

    if (remindLaterToggle) {
        remindLaterToggle.addEventListener('click', (e) => {
            e.preventDefault();
            if (remindLaterForm) {
                remindLaterForm.style.display = 'block';
                remindLaterToggle.parentElement.style.display = 'none';
                if (remindEmail) remindEmail.focus();
            }
        });
    }

    if (remindSubmitBtn && remindEmail) {
        remindSubmitBtn.addEventListener('click', async () => {
            const email = remindEmail.value.trim();

            if (!email || !email.includes('@')) {
                remindEmail.style.borderColor = '#dc2626';
                remindEmail.focus();
                return;
            }

            remindEmail.style.borderColor = '';
            const btnText = remindSubmitBtn.querySelector('.remind-btn-text');
            const btnLoading = remindSubmitBtn.querySelector('.remind-btn-loading');

            if (btnText) btnText.style.display = 'none';
            if (btnLoading) btnLoading.style.display = 'inline';
            remindSubmitBtn.disabled = true;
            ...
            try {
                const response = await fetch('/api/subscribe', {
                    ...
                    body: JSON.stringify({
                        email,
                        first_name: '',
                        utm_source: 'resumeradar_reminder',
                    }),
                });
                ...
                if (remindLaterForm) remindLaterForm.style.display = 'none';
                if (remindLaterSuccess) remindLaterSuccess.style.display = 'block';
                ...
                if (btnText) btnText.style.display = 'inline';
                if (btnLoading) btnLoading.style.display = 'none';
                remindSubmitBtn.disabled = false;
            }
        });

        // Allow Enter key to submit
        remindEmail.addEventListener('keydown', (e) => {
            if (e.key === 'Enter') {
                e.preventDefault();
                remindSubmitBtn.click();
            }
        });
    }
